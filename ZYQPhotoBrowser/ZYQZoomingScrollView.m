...
@interface ZYQZoomingScrollView ()
@property (nonatomic, weak) ZYQPhotoBrowser *photoBrowser;
- (void)handleSingleTap:(CGPoint)touchPoint;
- (void)handleDoubleTap:(CGPoint)touchPoint;
@end

@implementation ZYQZoomingScrollView
...
- (id)initWithPhotoBrowser:(ZYQPhotoBrowser *)browser {
    if ((self = [super init])) {
		...
		_tapView = [[ZYQTapDetectingView alloc] initWithFrame:self.bounds];
        ...
        if ([self conformsToProtocol:objc_getProtocol("ZYQZoomingScrollViewCustomProtocol")]) {
            ...
            tmpProgressView.trackTintColor    = browser.trackTintColor    ? self.photoBrowser.trackTintColor    : [UIColor colorWithWhite:0.2 alpha:1];
            tmpProgressView.progressTintColor = browser.progressTintColor ? self.photoBrowser.progressTintColor : [UIColor colorWithWhite:1.0 alpha:1];
...
- (void)setPhoto:(id<ZYQPhoto>)photo {
...
- (void)setProgress:(CGFloat)progress forPhoto:(ZYQPhoto*)photo {
    ZYQPhoto *p = (ZYQPhoto*)self.photo;

    if ([photo.photoURL.absoluteString isEqualToString:p.photoURL.absoluteString]) {
        if ([_progressView conformsToProtocol:objc_getProtocol("ZYQProgressViewProtocol")]) {
            if ([(UIView<ZYQProgressViewProtocol>*)_progressView zyq_progress] < progress) {
                [(UIView<ZYQProgressViewProtocol>*)_progressView setZyq_progress:progress];
            }
        }
        else{
#ifdef NODEPENDENCY
#else
            if ([(DACircularProgressView*)_progressView progress] < progress) {
                [(DACircularProgressView*)_progressView setProgress:progress animated:YES];
            }
#endif
        }
    }
